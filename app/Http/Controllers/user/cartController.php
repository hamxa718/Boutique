...
class cartController extends Controller
{
    public function addtocart($id)
    {
        ...
        if($product->stock == null || $product->stock == 0){
            return back()->with('delete','Product Stock Not Available');
        }else{
    ...
    public function addtocartwithqty(Request $request)
    {
        ...
        if($product->stock <= $request->qty ){
            return back()->with('delete','Your Enter quantity amount is more than stock');
        }else{
            $cart = Cart::add(['id' => $product->id, 'name' => $product->product_name, 'qty' => $request->qty, 'price' => $product->price, 'weight' => 0]);
